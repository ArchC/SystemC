...
: _mant( min_mant ), _r_flag( false )
{
	...
	if( a > 0 )
	{
	    _mant[1] = static_cast<word>( a );
	    _mant[2] = static_cast<word>( a >> bits_in_word );
	    _sign = 1;
	}
	else
	{
...
: _mant( a._mant ), _wp( a._wp ), _sign( a._sign ), _state( a._state ),
  _msw( a._msw ), _lsw( a._lsw ), _r_flag( false )
{}
...
{                                                                             \
    ...
    {                                                                         \
        _state = not_a_number;                                                \
	_mant.clear(); /* to avoid Purify UMRs during assignment */           \
        return;                                                               \
    }                                                                         \
}
...
scfx_rep::from_string( const char* s, int cte_wl )
{
        ...
        case 10:
	{
	    ...
	    resize_to( max( min_mant, n_word( 4 * length ) ) );
	  ...
	  int max_wl )
{
    ...
    int new_size = max( min_mant, len_lhs + len_rhs );
...
div__scfx_rep( const scfx_rep& lhs, const scfx_rep& rhs, int div_wl )
{
    ...
    result.resize_to( max( n_word( div_wl ) + 1, min_mant ) );
    result._mant.clear();
	...
	remainder.shift_left( 1 );
        remainder._lsw = remainder.find_lsw();
    }
       ...
       int& len_mant, scfx_mant_ref& lhs_mant, scfx_mant_ref& rhs_mant )
{
	...
	int lower_bound = min( lower_bound_lhs, lower_bound_rhs );
	int upper_bound = max( upper_bound_lhs, upper_bound_rhs );

	new_wp   = -lower_bound;
	len_mant = max( min_mant, upper_bound - lower_bound + 1 );
...
compare_msw_ff( const scfx_rep& lhs, const scfx_rep& rhs )
{
    ...
    int size = min( lhs_size, rhs_size );
