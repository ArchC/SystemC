...
class wif_trace {
public:
    wif_trace(const sc_string& name, const sc_string& wif_name);
...
wif_trace::wif_trace(const sc_string& name, const sc_string& wif_name)
        : name(name), wif_name(wif_name), bit_width(0)
{
    /* Intentionally blank */
}
...
public:
    wif_bool_trace(const bool& object, const sc_string& name,
...
wif_bool_trace::wif_bool_trace(const bool& object_, const sc_string& name,
    const sc_string& wif_name): wif_trace(name, wif_name), object(object_)
{
    bit_width = 1;
    old_value = object_;
    wif_type = "BIT";
...
public:
    wif_sc_bit_trace(const sc_bit& object, const sc_string& name,
                   const sc_string& wif_name);
    void write(FILE* f);
    bool changed();
...
wif_sc_bit_trace::wif_sc_bit_trace(const sc_bit& object_, const sc_string& name,
    const sc_string& wif_name): wif_trace(name, wif_name), object(object_)
{
    bit_width = 1;
    old_value = object_;
...
wif_sc_uint_base_trace::wif_sc_uint_base_trace(const sc_uint_base& _object, const sc_string& _name, const sc_string& _wif_name) 
  : wif_trace(_name, _wif_name), object(_object), old_value(_object.bitwidth())
{
    old_value = object;
    wif_type = "BIT";
}
...
template<class T>
class wif_T_trace: public wif_trace {
public:
    wif_T_trace(const T& object, const sc_string& name, const sc_string& wif_name,
		        wif_trace_file::wif_enum type)
    : wif_trace(name, wif_name), object(object), old_value(object)
    {  wif_type = wif_names[type]; }

    void write(FILE* f)
    {
       fprintf(f, "assign %s \"%s\" ;\n", wif_name.c_str(), object.to_string().c_str());
       old_value = object;
    }
...
typedef wif_T_trace<sc_bv_ns::sc_bv_base> wif_sc_bv_trace;
typedef wif_T_trace<sc_bv_ns::sc_lv_base> wif_sc_lv_trace;
...
void wif_trace_file::trace(const sc_bit& object, const sc_string& name)
{
    if(initialized)  
        wif_put_error_message("No traces can be added once simulation has started.\n"
                              "To add traces, create a new wif trace file.", false);
    ...
    sc_string temp_wif_name;
    create_wif_name(&temp_wif_name);
    traces.push_back(new wif_sc_bit_trace(object, name, temp_wif_name));
}
...
void wif_trace_file::trace(const sc_signal_resolved_vector& object, const sc_string& name)
{
    ...
    sc_string temp_wif_name;
    create_wif_name(&temp_wif_name);
    traces.push_back(new wif_sc_logic_vector_trace(static_cast<const sc_logic_vector&>(object), name, temp_wif_name));
}

void wif_trace_file::trace(const sc_bv_ns::sc_bv_base& object, const sc_string& name)
{
#if !defined(__SUNPRO_CC)
   traceT(object,name,WIF_BIT);
#else
   traceT(*this,object,name,WIF_BIT);
#endif
}

void wif_trace_file::trace(const sc_bv_ns::sc_lv_base& object, const sc_string& name)
{
#if !defined(__SUNPRO_CC)
   traceT(object,name,WIF_MVL);
#else
   traceT(*this,object,name,WIF_MVL);
#endif
}
...
{
  return sc_string("0")+sc_string(wif_name_index++);
}
