...
protected:
    ...
    // Sun's SC5 doesn't implement member templates
    template<class T>
...
// Sun's SC5 doesn't implement member templates
template<class T>
void traceT(wif_trace_file& trace_file, const T& object, const sc_string& name, 
            wif_trace_file::wif_enum type)
{
   if(trace_file.initialized)
     ...
     trace_file.traces.push_back(new wif_T_trace<T>(object, name,
       trace_file.obtain_new_index(),type));
}
#endif
